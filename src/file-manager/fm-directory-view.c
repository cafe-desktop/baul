...
#include <libbaul-private/baul-recent.h>
...
#include <libbaul-private/baul-desktop-directory.h>
...
#include <libbaul-private/baul-file-private.h> /* for baul_file_get_existing_by_uri */
...
#include <libbaul-private/baul-undostack-manager.h>
...
#include "libbaul-private/baul-open-with-dialog.h"
...
struct FMDirectoryViewDetails
{
	...
	CtkActionGroup *scripts_action_group;
	guint scripts_merge_id;

	GList *templates_directory_list;
	CtkActionGroup *templates_action_group;
	 ...
	 * losing focus when the underlying CtkTreeView is updated.
	 */
	...
	CtkActionGroup *open_with_action_group;
...
static void     fm_directory_view_create_links_for_files       (FMDirectoryView      *view,
								GList                *files,
								GArray               *item_locations);
static void     trash_or_delete_files                          (CtkWindow            *parent_window,
								const GList          *files,
...
static void     fm_directory_view_load_location                (BaulView         *baul_view,
								const char           *location);
static void     fm_directory_view_stop_loading                 (BaulView         *baul_view);
...
static void     metadata_for_directory_as_file_ready_callback  (BaulFile         *file,
								gpointer              callback_data);
static void     metadata_for_files_in_directory_ready_callback (BaulDirectory    *directory,
								GList                *files,
								gpointer              callback_data);
static void     fm_directory_view_trash_state_changed_callback (BaulTrashMonitor *trash,
								gboolean              state,
								gpointer              callback_data);
...
static void action_open_scripts_folder_callback    (CtkAction *action,
						    gpointer   callback_data);
static void action_cut_files_callback              (CtkAction *action,
						    gpointer   callback_data);
static void action_copy_files_callback             (CtkAction *action,
						    gpointer   callback_data);
static void action_paste_files_callback            (CtkAction *action,
						    gpointer   callback_data);
static void action_copy_to_next_pane_callback      (CtkAction *action,
						    gpointer   callback_data);
static void action_move_to_next_pane_callback      (CtkAction *action,
						    gpointer   callback_data);
static void action_rename_callback                 (CtkAction *action,
						    gpointer   callback_data);
static void action_rename_select_all_callback      (CtkAction *action,
						    gpointer   callback_data);
static void action_paste_files_into_callback       (CtkAction *action,
						    gpointer   callback_data);
static void action_connect_to_server_link_callback (CtkAction *action,
						    gpointer   data);
static void action_mount_volume_callback           (CtkAction *action,
						    gpointer   data);
static void action_unmount_volume_callback         (CtkAction *action,
						    gpointer   data);
static void action_format_volume_callback          (CtkAction *action,
						    gpointer   data);
static void action_start_volume_callback           (CtkAction *action,
						    gpointer   data);
static void action_stop_volume_callback            (CtkAction *action,
						    gpointer   data);
static void action_detect_media_callback           (CtkAction *action,
						    gpointer   data);

/* location popup-related actions */

static void action_location_open_alternate_callback (CtkAction *action,
						     gpointer   callback_data);
static void action_location_open_folder_window_callback (CtkAction *action,
							 gpointer   callback_data);

static void action_location_cut_callback            (CtkAction *action,
						     gpointer   callback_data);
static void action_location_copy_callback           (CtkAction *action,
						     gpointer   callback_data);
static void action_location_trash_callback          (CtkAction *action,
						     gpointer   callback_data);
static void action_location_delete_callback         (CtkAction *action,
						     gpointer   callback_data);
static void action_location_properties_callback     (CtkAction *action,
						     gpointer   callback_data);
...
static void undo_redo_menu_update_callback (BaulUndoStackManager* manager, gpointer arg1, gpointer data);
...
static void action_undo_callback (CtkAction *action, gpointer callback_data);
			           ...
			           FMDirectoryView *directory_view)
{
	...
	result->files = baul_file_list_copy (files);
...
application_launch_parameters_free (ApplicationLaunchParameters *parameters)
{
	...
	baul_file_list_free (parameters->files);
...
file_and_directory_list_to_files (GList *fad_list)
{
	...
	for (l = fad_list; l != NULL; l = l->next) {
		fad = l->data;
		res = g_list_prepend (res, baul_file_ref (fad->file));
	}
...
static GList *
file_and_directory_list_from_files (BaulDirectory *directory, GList *files)
{
	GList *res, *l;
	FileAndDirectory *fad = NULL;
	...
	for (l = files; l != NULL; l = l->next) {
		...
		fad->directory = baul_directory_ref (directory);
		fad->file = baul_file_ref (l->data);
...
{
	baul_directory_unref (fad->directory);
	baul_file_unref (fad->file);
	g_free (fad);
}
...
script_launch_parameters_new (BaulFile *file,
			      FMDirectoryView *directory_view)
{
...
{
	g_object_unref (parameters->directory_view);
	baul_file_unref (parameters->file);
	g_free (parameters);
}

static CreateTemplateParameters *
create_template_parameters_new (BaulFile *file,
				FMDirectoryView *directory_view)
{
...
BaulWindowInfo *
fm_directory_view_get_baul_window (FMDirectoryView  *view)
{
...
BaulWindowSlotInfo *
fm_directory_view_get_baul_window_slot (FMDirectoryView  *view)
{
...
/* Returns the CtkWindow that this directory view occupies, or NULL
 * if at the moment this directory view is not in a CtkWindow or the
 * CtkWindow cannot be determined. Primarily used for parenting dialogs.
 */
CtkWindow *
...
static gboolean
fm_directory_view_confirm_multiple (CtkWindow *parent_window,
				    int count,
				    gboolean tabs)
{
	CtkDialog *dialog;
...
get_view_directory (FMDirectoryView *view)
{
	...
	uri = baul_directory_get_uri (view->details->model);
	if (eel_uri_is_desktop (uri)) {
		g_free (uri);
		uri = baul_get_desktop_directory_uri ();
				  ...
				  gboolean confirm_multiple)
{
	...
	baul_mime_activate_files (fm_directory_view_get_containing_window (view),
				      view->details->slot,
				      files,
				      path,
				      mode,
				 ...
				 BaulWindowOpenFlags flags)
{
	...
	path = get_view_directory (view);
	baul_mime_activate_file (fm_directory_view_get_containing_window (view),
				     view->details->slot,
				     file,
				     path,
				     mode,
...
delete_selected_files (FMDirectoryView *view)
{
	...
	baul_file_operations_delete (locations, fm_directory_view_get_containing_window (view), NULL, NULL);
				    ...
				    gpointer   callback_data)
{
	...
	selection = fm_directory_view_get_selection_for_file_transfer (view);
	baul_restore_files_from_trash (selection,
					   fm_directory_view_get_containing_window (view));
...
static void
pattern_select_response_cb (CtkWidget *dialog, int response, gpointer user_data)
{
	FMDirectoryView *view;
	BaulDirectory *directory;
	CtkWidget *entry;
...
static void
query_name_entry_changed_cb  (CtkWidget *entry, CtkWidget *button)
{
	const char *text;
	gboolean sensitive;
				...
				gpointer   callback_data)
{
	...
	if (directory_view->details->model &&
	    BAUL_IS_SEARCH_DIRECTORY (directory_view->details->model)) {
		BaulSearchDirectory *search;
		CtkWidget *dialog, *grid, *label, *entry, *chooser, *save_button;
		...
		if (ctk_dialog_run (CTK_DIALOG (dialog)) == CTK_RESPONSE_OK) {
			...
			baul_search_directory_save_to_file (search, uri);
...
we_are_in_vfolder_desktop_dir (FMDirectoryView *view)
{
	...
	file = baul_directory_get_corresponding_file (view->details->model);
	mime_type = baul_file_get_mime_type (file);
	baul_file_unref (file);
...
static void set_up_scripts_directory_global(void)
{
	...
	scripts_directory_path = g_build_filename (g_get_user_config_dir (), "baul", "scripts", NULL);
	...
	if (g_mkdir_with_parents(scripts_directory_path, 0755) == 0)
	{
		...
		{
			char* nautilus_syslink = g_build_filename(g_get_user_config_dir(), "baul", "scripts", "nautilus", NULL);
				 ...
				 GCallback changed_callback)
{
	...
	if (g_list_find (*directory_list, directory) == NULL) {
		...
		baul_directory_file_monitor_add (directory, directory_list,
						     FALSE, attributes,
						     (BaulDirectoryCallback)changed_callback, view);
				      ...
				      GCallback changed_callback)
{
	*directory_list = g_list_remove	(*directory_list, directory);
	...
	baul_directory_file_monitor_remove (directory, directory_list);
...
static void
fm_directory_view_grab_focus (BaulView *view)
{
...
static void
view_iface_update_menus (BaulView *view)
{
...
static int
fm_directory_view_get_selection_count (BaulView *view)
{
...
static GList *
fm_directory_view_get_selection_locations (BaulView *view)
{
	GList *files;
	GList *locations;
	GList *l;
...
file_list_from_location_list (const GList *uri_list)
{
	...
	for (node = uri_list; node != NULL; node = node->next) {
		file_list = g_list_prepend
			(file_list,
			 baul_file_get (node->data));
	}
	return g_list_reverse (file_list);
}

static void
fm_directory_view_set_selection_locations (BaulView *baul_view,
					   GList *selection_locations)
{
	FMDirectoryView *view;

	view = FM_DIRECTORY_VIEW (baul_view);
...
void
fm_directory_view_init_view_iface (BaulViewIface *iface)
{
...
{
	BaulDirectory *scripts_directory;
	...
	baul_directory_unref (scripts_directory);

	if (baul_should_use_templates_directory ()) {
		BaulDirectory *templates_directory;
		char *templates_uri;

		templates_uri = baul_get_templates_directory_uri ();
		templates_directory = baul_directory_get_by_uri (templates_uri);
		g_free (templates_uri);
		add_directory_to_templates_directory_list (view, templates_directory);
		baul_directory_unref (templates_directory);
	}
	...
	g_signal_connect_object (baul_clipboard_monitor_get (), "clipboard_changed",
	...
	BaulUndoStackManager* manager = baul_undostack_manager_instance ();

	g_signal_connect_object (G_OBJECT(manager), "request-menu-update",
		   G_CALLBACK(undo_redo_menu_update_callback), view, 0);

	baul_undostack_manager_request_menu_update (baul_undostack_manager_instance());
}
...
static void
fm_directory_view_destroy (CtkWidget *object)
{
	FMDirectoryView *view;
	GList *node, *next;
	...
	if (view->details->model) {
		baul_directory_unref (view->details->model);
		view->details->model = NULL;
	}

	if (view->details->directory_as_file) {
		baul_file_unref (view->details->directory_as_file);
...
fm_directory_view_display_selection_info (FMDirectoryView *view)
{
	...
	for (p = selection; p != NULL; p = p->next) {
		...
		if (baul_file_is_directory (file)) {
			folder_count++;
			if (baul_file_get_directory_item_count (file, &file_item_count, NULL)) {
			...
			non_folder_count++;
			if (!baul_file_can_get_size (file)) {
				non_folder_size_known = TRUE;
				non_folder_size += baul_file_get_size (file);
			}
		}

		if (first_item_name == NULL) {
			first_item_name = baul_file_get_display_name (file);
		}
	}
	...
	free_space_str = baul_file_get_volume_free_space (view->details->directory_as_file);
	...
	baul_window_slot_info_set_status (view->details->slot,
					      status_string);
...
{
	baul_window_info_report_selection_changed (view->details->window);
...
static void
fm_directory_view_load_location (BaulView *baul_view,
				 const char *location)
{
	BaulDirectory *directory;
	FMDirectoryView *directory_view;

	directory_view = FM_DIRECTORY_VIEW (baul_view);
	...
	directory = baul_directory_get_by_uri (location);
	load_directory (directory_view, directory);
	baul_directory_unref (directory);
}

static void
fm_directory_view_stop_loading (BaulView *baul_view)
{
	fm_directory_view_stop (FM_DIRECTORY_VIEW (baul_view));
}
	...
	if (view->details->window != NULL) {
		...
		if (locations_selected != NULL && all_files_seen) {
			...
			if (FM_IS_LIST_VIEW (view)) {
				 ...
				 * but at this point the CtkTreeView hasn't allocated the new nodes
...
debuting_files_data_free (DebutingFilesData *data)
{
	...
	baul_file_list_free (data->added_files);
	g_free (data);
}
				  ...
				  DebutingFilesData *data)
{
	...
	location = baul_file_get_location (new_file);
...
/* This needs to be called prior to baul_file_operations_copy_move.
...
static gboolean
real_file_still_belongs (FMDirectoryView *view, BaulFile *file, BaulDirectory *directory)
{
	...
	return baul_directory_contains_file (directory, file);
}

static gboolean
still_should_show_file (FMDirectoryView *view, BaulFile *file, BaulDirectory *directory)
{
...
display_pending_files (FMDirectoryView *view)
{
	...
	if (view->details->model != NULL
	    && baul_directory_are_all_files_seen (view->details->model)
		     ...
		     GList **pending_list)
{
	...
	*pending_list = g_list_concat (file_and_directory_list_from_files (directory, files),
	...
	if (! view->details->loading || baul_directory_are_all_files_seen (directory)) {
...
static void
files_added_callback (BaulDirectory *directory,
		      GList *files,
		      gpointer callback_data)
{
	FMDirectoryView *view;
...
real_load_error (FMDirectoryView *view, GError *error)
{
	 ...
	 * point of view, not from the BaulDirectory point of view).
				       ...
				       BaulDirectory*directory)
{
	...
	baul_directory_file_monitor_remove (directory, &view->details->model);
 ...
 **/
static gboolean
fm_directory_view_can_rename_file (FMDirectoryView *view, BaulFile *file)
{
	return EEL_CALL_METHOD_WITH_RETURN_VALUE
		(FM_DIRECTORY_VIEW_CLASS, view,
 ...
 *
 * Get a list of BaulFile pointers that represents the
 ...
 *
 * Return value: GList of BaulFile pointers representing the selection.
 *
 **/
GList *
...
fm_directory_view_get_ui_manager (FMDirectoryView  *view)
{
	...
	return baul_window_info_get_ui_manager (view->details->window);
}
 ...
 *
 * Return value: BaulDirectory for this view.
 *
 **/
BaulDirectory *
...
prepend_uri_one (gpointer data, gpointer callback_data)
{
	...
	file = (BaulFile *) data;
	*result = g_list_prepend (*result, baul_file_get_uri (file));
}
		       ...
		       FMDirectoryView *view)
{
	...
	baul_file_operations_trash_or_delete (locations,
						  parent_window,
						  (BaulDeleteCallback) trash_or_delete_done_cb,
						  view);
	g_list_free_full (locations, g_object_unref);
}
...
delayed_rename_file_hack_removed (RenameData *data)
{
	...
	baul_file_unref (data->new_file);
	g_free (data);
}


static void
rename_file (FMDirectoryView *view, BaulFile *new_file)
{
	/* HACK!!!!
	   ...
	   the model changes CtkTreeView clears the editing. This hack just
	...
	*/
	if (FM_IS_LIST_VIEW (view)) {
		...
		data->new_file = baul_file_ref (new_file);
		if (view->details->delayed_rename_file_id != 0) {
			g_source_remove (view->details->delayed_rename_file_id);
		}
			     ...
			     gpointer         user_data)
{
	...
	g_hash_table_insert (data->added_locations, baul_file_get_location (new_file), NULL);
}
			    ...
			    BaulFile *source)
{
	...
	g_return_if_fail (baul_file_is_local (source));
...
static void
add_submenu (CtkUIManager *ui_manager,
				...
				const char *popup_placeholder)
{
	...
	uri = baul_file_get_parent_uri (file);
...
static void
add_x_content_apps (FMDirectoryView *view, BaulFile *file, GList **applications)
{
	GMount *mount;
	...
	x_content_types = baul_autorun_get_cached_x_content_types_for_mount (mount);
...
reset_open_with_menu (FMDirectoryView *view, GList *selection)
{
	...
	baul_ui_prepare_merge_ui (ui_manager,
				      "OpenWithGroup",
				      &view->details->open_with_merge_id,
				      &view->details->open_with_action_group);
	...
	default_app = NULL;
	if (filter_default) {
		default_app = baul_mime_get_default_application_for_files (selection);
	}

	applications = NULL;
	if (other_applications_visible) {
		applications = baul_mime_get_applications_for_files (selection);
	}
...
static GList *
get_all_extension_menu_items (CtkWidget *window,
			      GList *selection)
{
	GList *items;
	GList *providers;
...
extension_action_callback_data_free (ExtensionActionCallbackData *data)
{
	...
	baul_file_list_free (data->selection);
...
search_in_menu_items (GList* items, const char *item_name)
{
	...
	for (list = items; list != NULL; list = list->next) {
		...
		if (menu != NULL) {
			...
			submenus = baul_menu_get_items (menu);
			ret = search_in_menu_items (submenus, item_name);
			baul_menu_item_list_free (submenus);
			g_object_unref (menu);
			if (ret) {
			    return TRUE;
			}
			   ...
			   gpointer   callback_data)
{
	...
	if (is_valid) {
		baul_menu_item_activate (data->item);
	}
}
...
static cairo_surface_t *
get_menu_icon_for_file (BaulFile  *file,
			CtkWidget *widget)
{
	BaulIconInfo *info;
	cairo_surface_t *surface;
				...
				GList *files)
{
	...
	data->view = view;
	data->selection = baul_file_list_copy (files);
	data->action = action;
				       ...
				       char **file_paths, char **uris, char **uri)
{
	 ...
	 * baul_directory_is_local returns FALSE for nfs.
	 */
	directory_uri = baul_directory_get_uri (view->details->model);
...
get_directory_view_of_extra_pane (FMDirectoryView *view)
{
	...
	slot = baul_window_info_get_extra_slot (fm_directory_view_get_baul_window (view));
	if (slot != NULL) {
		BaulView *next_view;

		next_view = baul_window_slot_info_get_current_view (slot);
 ...
 * to take advantage of the current Baul state.
 */
static void set_script_environment_variables(FMDirectoryView* view, GList* selected_files)
{
	...
	if (next_view)
	{
		...
		baul_file_list_free(next_pane_selected_files);
	}
	else
	{
		file_paths = g_strdup("");
		     ...
		     gpointer   callback_data)
{
	...
	file_uri = baul_file_get_uri (launch_parameters->file);
	...
	baul_file_list_free (selected_files);
...
static gboolean
update_directory_in_scripts_menu (FMDirectoryView *view, BaulDirectory *directory)
{
	...
	filtered = baul_file_list_filter_hidden (file_list, FALSE);
	baul_file_list_free (file_list);

	file_list = baul_file_list_sort_by_display_name (filtered);
	...
	for (node = file_list; node != NULL; node = node->next) {
		...
		if (baul_file_is_launchable (file)) {
			...
			if (directory_belongs_in_scripts_menu (uri)) {
				dir = baul_directory_get_by_uri (uri);
				add_directory_to_scripts_directory_list (view, dir);
				baul_directory_unref (dir);
...
update_scripts_menu (FMDirectoryView *view)
{
	...
	any_scripts = FALSE;
	sorted_copy = baul_directory_list_sort_by_uri
		(baul_directory_list_copy (view->details->scripts_directory_list));
				 ...
				 const char *popup_bg_path)
{
	...
	tmp = baul_file_get_display_name (file);
				    ...
				    BaulDirectory *directory)
{
	...
	for (node = file_list; num < TEMPLATE_LIMIT && node != NULL; node = node->next, num++) {
		...
		if (baul_file_is_directory (file)) {
			...
			g_free (uri);
		} else if (baul_file_can_read (file)) {
...
update_templates_menu (FMDirectoryView *view)
{
	...
	if (baul_should_use_templates_directory ()) {
		templates_directory_uri = baul_get_templates_directory_uri ();
	} else {
	...
	any_templates = FALSE;
	sorted_copy = baul_directory_list_sort_by_uri
		(baul_directory_list_copy (view->details->templates_directory_list));
	for (node = sorted_copy; node != NULL; node = node->next) {
		char *uri;
		   ...
		   gboolean         cut)
{
	...
	if (count == 1) {
		...
		name = baul_file_get_display_name (clipboard_contents->data);
		if (cut) {
				 ...
				 char *target_uri)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		uris = g_list_prepend (uris,
				       baul_file_get_uri ((BaulFile *) l->data));
	}
	uris = g_list_reverse (uris);
					...
					gpointer          callback_data)
{
	...
	if (view->details->window != NULL) {
		...
		directory_uri = baul_file_get_activation_uri (data->target);
		    ...
		    gboolean select_all)
{
	...
	if (selection_not_empty_in_menu_callback (view, selection)) {
		...
		if (!select_all) {
			...
			select_all = baul_file_is_directory (file);
		}
				 ...
				 gpointer   callback_data)
{
	...
	baul_file_operations_delete (files, fm_directory_view_get_containing_window (view),
					 NULL, NULL);
					     ...
					     gpointer   callback_data)
{
	...
	l.next = NULL;
	l.data = file;
	baul_restore_files_from_trash (&l,
					   fm_directory_view_get_containing_window (view));
}
...
static const CtkActionEntry directory_view_entries[] = {
...
static gboolean
can_paste_into_file (BaulFile *file)
{
	if (baul_file_is_directory (file) &&
	    baul_file_can_write (file)) {
		return TRUE;
	}
	if (baul_file_has_activation_uri (file)) {
		GFile *location;
		BaulFile *activation_file;
		gboolean res;

		location = baul_file_get_activation_location (file);
		activation_file = baul_file_get (location);
		g_object_unref (location);

		/* The target location might not have data for it read yet,
		   and we can't want to do sync I/O, so treat the unknown
		   case as can-write */
		res = (baul_file_get_file_type (activation_file) == G_FILE_TYPE_UNKNOWN) ||
			(baul_file_get_file_type (activation_file) == G_FILE_TYPE_DIRECTORY &&
			 baul_file_can_write (activation_file));

		baul_file_unref (activation_file);
...
file_list_all_are_folders (GList *file_list)
{
	...
	for (l = file_list; l != NULL; l = l->next) {
		...
		if (baul_file_is_baul_link (file) &&
			...
			linked_file = baul_file_get_existing_by_uri (activation_uri);
			...
			is_dir =
				(linked_file != NULL &&
				 baul_file_is_directory (linked_file)) ||
				(activation_uri != NULL &&
				 activation_uri[strlen (activation_uri) - 1] == '/');

			baul_file_unref (linked_file);
			g_free (activation_uri);
...
static void
file_should_show_foreach (BaulFile        *file,
			  gboolean            *show_mount,
			  ...
			  GDriveStartStopType *start_stop_type)
{
	...
	*show_poll = FALSE;
	...
	if (baul_file_can_unmount (file) && !*show_eject && !*show_stop) {
		*show_unmount = TRUE;
	}
	...
	*start_stop_type = baul_file_get_start_stop_type (file);
...
static void
file_should_show_self (BaulFile        *file,
		       gboolean            *show_mount,
		       gboolean            *show_unmount,
		       gboolean            *show_eject,
		       gboolean            *show_format,
...
static void
update_restore_from_trash_action (CtkAction *action,
				  GList *files,
				  gboolean is_self)
{
	BaulFile *original_file;
	BaulFile *original_dir;
	...
	if (files != NULL) {
		if (g_list_length (files) == 1) {
			original_file = baul_file_get_trash_original_file (files->data);
		} else {
			original_dirs_hash = baul_trashed_files_get_original_directories (files, NULL);
	...
	if (original_file != NULL || original_dirs != NULL) {
		...
		if (original_file != NULL) {
			...
			original_location = baul_file_get_location (original_dir);
		}
...
real_update_menus (FMDirectoryView *view)
{
	...
	for (l = selection; l != NULL; l = l->next) {
		...
		if (!baul_mime_file_opens_in_external_app (file)) {
			show_app = FALSE;
		}
	...
	if (can_open && show_app) {
		app = baul_mime_get_default_application_for_files (selection);
	}
...
static void
location_popup_file_attributes_ready (BaulFile *file,
				      gpointer      data)
{
	FMDirectoryView *view;
...
unschedule_pop_up_location_context_menu (FMDirectoryView *view)
{
	if (view->details->location_popup_directory_as_file != NULL) {
		...
		baul_file_cancel_call_when_ready (view->details->location_popup_directory_as_file,
						      ...
						      view);
		baul_file_unref (view->details->location_popup_directory_as_file);
						...
						const char      *location)
{
	...
	if (location != NULL) {
		file = baul_file_get_by_uri (location);
	} else {
		file = baul_file_ref (view->details->directory_as_file);
	}
...
static void
file_changed_callback (BaulFile *file, gpointer callback_data)
{
...
static void
metadata_for_directory_as_file_ready_callback (BaulFile *file,
			      		       gpointer callback_data)
{
	FMDirectoryView *view;
...
disconnect_model_handlers (FMDirectoryView *view)
{
	...
	baul_file_cancel_call_when_ready (view->details->directory_as_file,
					      metadata_for_directory_as_file_ready_callback,
					      view);
	baul_directory_cancel_callback (view->details->model,
					    metadata_for_files_in_directory_ready_callback,
					    view);
	baul_directory_file_monitor_remove (view->details->model,
						&view->details->model);
	baul_file_monitor_remove (view->details->directory_as_file,
				      &view->details->directory_as_file);
}
...
static void
fm_directory_view_select_file (FMDirectoryView *view, BaulFile *file)
{
	GList file_list;
...
fm_directory_view_stop (FMDirectoryView *view)
{
	...
	if (view->details->model != NULL) {
		baul_directory_file_monitor_remove (view->details->model, view);
	}
...
fm_directory_view_is_editable (FMDirectoryView *view)
{
	...
	if (directory != NULL) {
		return baul_directory_is_editable (directory);
	}
...
{
	if (view->details->window != NULL) {
		baul_window_info_set_initiated_unmount(view->details->window,
							   initiated_unmount);
	}
}
...
real_is_read_only (FMDirectoryView *view)
{
	...
	file = fm_directory_view_get_directory_as_file (view);
	if (file != NULL) {
		return !baul_file_can_write (file);
	}
	return FALSE;
}
 ...
 */
gboolean
fm_directory_view_should_show_file (FMDirectoryView *view, BaulFile *file)
{
...
fm_directory_view_get_uri (FMDirectoryView *view)
{
	...
	return baul_directory_get_uri (view->details->model);
}
				   ...
				   FMDirectoryView *view)
{
	...
	target_file = baul_file_get_existing_by_uri (target_uri);
	...
	if (target_file != NULL && baul_file_is_launcher (target_file)) {
		...
		/* Handle dropping onto a grapa archiver file, instead of starting a move/copy */

		baul_file_unref (target_file);

		quoted_uri = g_shell_quote (target_uri);
		command = g_strconcat ("grapa -a ", quoted_uri, NULL);
		g_free (quoted_uri);
...
gboolean
fm_directory_view_can_accept_item (BaulFile *target_item,
				   const char *item_uri,
				   FMDirectoryView *view)
{
	...
	return baul_drag_can_accept_item (target_item, item_uri);
}
...
static void
fm_directory_view_parent_set (CtkWidget *widget,
...
fm_directory_view_class_init (FMDirectoryViewClass *klass)
{
	...
	/* Get rid of the strange 3-pixel gap that CtkScrolledWindow
				...
				gpointer              data)
{
	...
	BaulUndoStackMenuData* menudata = (BaulUndoStackMenuData*) arg;
