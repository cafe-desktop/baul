/* -*- Mode: C; indent-tabs-mode: t; c-basic-offset: 8; tab-width: 8 -*-
   ...
   baul-directory.h: Caja directory model.
 ...
 */
CajaDirectory *baul_directory_get                      (GFile                     *location);
CajaDirectory *baul_directory_get_by_uri               (const char                *uri);
CajaDirectory *baul_directory_get_for_file             (CajaFile              *file);

/* Covers for g_object_ref and g_object_unref that provide two conveniences:
 * 1) Using these is type safe.
 * 2) You are allowed to call these with NULL,
 */
CajaDirectory *baul_directory_ref                      (CajaDirectory         *directory);
void               baul_directory_unref                    (CajaDirectory         *directory);

/* Access to a URI. */
char *             baul_directory_get_uri                  (CajaDirectory         *directory);
GFile *            baul_directory_get_location             (CajaDirectory         *directory);

/* Is this file still alive and in this directory? */
gboolean           baul_directory_contains_file            (CajaDirectory         *directory,
        CajaFile              *file);

/* Get (and ref) a CajaFile object for this directory. */
CajaFile *     baul_directory_get_corresponding_file   (CajaDirectory         *directory);

/* Waiting for data that's read asynchronously.
 * The file attribute and metadata keys are for files in the directory.
 */
void               baul_directory_call_when_ready          (CajaDirectory         *directory,
        CajaFileAttributes     file_attributes,
        gboolean                   wait_for_all_files,
        CajaDirectoryCallback  callback,
        gpointer                   callback_data);
void               baul_directory_cancel_callback          (CajaDirectory         *directory,
        CajaDirectoryCallback  callback,
        gpointer                   callback_data);


/* Monitor the files in a directory. */
void               baul_directory_file_monitor_add         (CajaDirectory         *directory,
        gconstpointer              client,
        gboolean                   monitor_hidden_files,
        CajaFileAttributes     attributes,
        CajaDirectoryCallback  initial_files_callback,
        gpointer                   callback_data);
void               baul_directory_file_monitor_remove      (CajaDirectory         *directory,
        gconstpointer              client);
void               baul_directory_force_reload             (CajaDirectory         *directory);

/* Get a list of all files currently known in the directory. */
GList *            baul_directory_get_file_list            (CajaDirectory         *directory);

GList *            baul_directory_match_pattern            (CajaDirectory         *directory,
        const char *glob);
 ...
 */
gboolean           baul_directory_are_all_files_seen       (CajaDirectory         *directory);

/* Return true if the directory is local. */
gboolean           baul_directory_is_local                 (CajaDirectory         *directory);

gboolean           baul_directory_is_in_trash              (CajaDirectory         *directory);
 ...
 */
gboolean           baul_directory_is_not_empty             (CajaDirectory         *directory);
...
gboolean           baul_directory_is_desktop_directory     (CajaDirectory         *directory);

gboolean           baul_directory_is_editable              (CajaDirectory         *directory);
